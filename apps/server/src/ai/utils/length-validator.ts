...
export class ContentLengthValidator implements ValidatorConstraintInterface {
  validate(body: string, args: ValidationArguments) {
    ...
    const length = getContentBodyLength(body);
    return isValidBodyLength(length, maxLength);
  }
...
const getContentBodyLength = (body: string) => {
  const regex = /!?\[(\w+)\]\([^)]+\)/g;
...
const isValidBodyLength = (body: number, restrictNumber: number) => {
